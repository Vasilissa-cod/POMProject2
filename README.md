...
Приложение "Система бронирования авиабилетов" состоит из нескольких страниц, которые тестируются в рамках проекта:

### Основной URL приложения
- **v01 (версия без ошибок):** `https://slqamsk.github.io/cases/slflights/v01/`
- **v02 (версия с ошибками):** `https://slqamsk.github.io/cases/slflights/v02/`

### Тестируемые страницы/разделы:

1. **Страница аутентификации (LoginPage)**
   - URL: базовый URL приложения (при первом открытии)
   - Функционал: логин пользователя, валидация учетных данных

2. **Страница поиска рейсов (SearchPage)**
   - URL: базовый URL приложения (после успешного логина)
   - Функционал: поиск авиабилетов по параметрам (город отправления, город прибытия, дата вылета)

3. **Страница со списком найденных рейсов (FlightsListPage)**
   - URL: открывается после выполнения поиска
   - Функционал: отображение списка найденных рейсов, сортировка, выбор рейса для регистрации

4. **Страница регистрации на рейс (RegistrationPage)**
   - URL: открывается при выборе конкретного рейса
   - Функционал: заполнение данных пассажира (ФИО, номер паспорта, email, телефон), валидация данных

Все страницы находятся в рамках одного SPA-приложения и навигация между ними происходит динамически через JavaScript.
